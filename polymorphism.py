...
student_m = Student('Василий', 'Петров', 'M')
student_m.courses_in_progress += ['Python']
student_m.finished_courses += ['Java']

student_f = Student('Лола', 'Васильева', 'F')
student_f.courses_in_progress += ['Python']
student_f.finished_courses += ['Golang']

reviewer_m = Reviewer('Михаил', 'Светлов')
reviewer_m.courses_attached += ['Python']

reviewer_f = Reviewer('Татьяна', 'Лавочкина')
reviewer_f.courses_attached += ['Python']

lector_m = Lectures('Сергей', 'Степанов')
lector_m.courses_attached += ['Python']

lector_f = Lectures('Виктория', 'Егорова')
lector_f.courses_attached += ['Python']

reviewer_f.rate_hw(student_f, 'Python', 6)
reviewer_f.rate_hw(student_m, 'Python', 7)
reviewer_m.rate_hw(student_f, 'Python', 10)
reviewer_m.rate_hw(student_m, 'Python', 8)

student_f.rate_lc(lector_m, 'Python', 9)
student_f.rate_lc(lector_f, 'Python', 10)
student_m.rate_lc(lector_m, 'Python', 6)
